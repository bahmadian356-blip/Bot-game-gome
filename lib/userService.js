...
async function getOrCreateProfile(telegramUser) {
  ...
  if (existing) {
    const { data: updated, error: updateErr } = await supabase
      .from('profiles')
      .update({ first_name, last_name, username, photo_url, updated_at: new Date().toISOString() })
      .eq('telegram_id', id)
      .select()
      .single();
  ...
  const { data: created, error: insertErr } = await supabase
    .from('profiles')
    .insert({
      telegram_id: id, first_name, last_name, username, photo_url,
      xp: 0, level: 1, games_played: 0, wins: 0, losses: 0,
    })
    .select()
    .single();
...
async function applyGameResult(telegramId, didWin) {
  const { data: profile, error: selectErr } = await supabase
    .from('profiles').select('*').eq('telegram_id', telegramId).single();
  if (selectErr) throw selectErr;
